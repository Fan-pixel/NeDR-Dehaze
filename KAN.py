...
# KAN_CID
class KAN_CID(nn.Module):
    def __init__(self, f_number, padding_mode='reflect') -> None:
        super().__init__()
        self.channel_independent = DConv7(f_number, padding_mode)
        self.channel_dependent = KAN(f_number, hidden_features=f_number, out_features=f_number)

    def forward(self, x):
        # 通道独立卷积
        x = self.channel_independent(x)  # 输出形状: (B, C, H, W)

        # 重塑为二维张量
        B, C, H, W = x.shape
        x = x.permute(0, 2, 3, 1).contiguous()  # 重塑为 (B, H, W, C)
        x = x.view(B * H * W, C)  # 重塑为 (B * H * W, C)

        # 通道依赖特征提取
        x = self.channel_dependent(x)  # 输出形状: (B * H * W, C)

        # 重塑回四维张量
        x = x.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()  # 重塑为 (B, C, H, W)
...
def _trunc_normal_(tensor, mean, std, a, b):
    ...
    with torch.no_grad():
        ...
        # Uniformly fill tensor with values on [l, u], then translate to
        # [2l-1, 2u-1].
